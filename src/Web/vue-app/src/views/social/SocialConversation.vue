        ...
        <div
          :class="[
            ...
            msg.senderMemberId === currentMemberId
              ? 'bg-[#be1e2c] text-white'
              : 'bg-gray-100 text-gray-900'
          ]"
        >
          {{ msg.content }}
          <p :class="['mt-0.5 text-[10px]', msg.senderMemberId === currentMemberId ? 'text-red-200' : 'text-gray-400']">
    ...
    <div class="border-t border-gray-200 px-4 py-3">
      <div class="flex gap-2">
        <input
          v-model="newMessage"
          type="text"
          class="flex-1 rounded-full border border-gray-300 px-4 py-2 text-sm focus:border-[#be1e2c] focus:outline-none focus:ring-1 focus:ring-[#be1e2c]"
        ...
        <button
          ...
          class="flex h-10 w-10 items-center justify-center rounded-full bg-[#be1e2c] text-white transition hover:bg-[#a01825] disabled:opacity-50"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
